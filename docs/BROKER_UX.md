...
- **Sessions**: a "Registered Visual Studio sessions" table (solution name, PID, debugger mode, active document, last seen, age, solution path, session id, advertised capabilities) that shows a contextual "No sessions yet" message in place of the table when nothing is registered.
