...
- **Usage analytics**: disabled by default. When enabled, the broker records aggregate-only usage counters in `%LOCALAPPDATA%\NetVsMcp\Analytics\analytics.db`; Debug builds use an `Analytics\analytics.db` folder beside the executable so they can run beside an installed Release broker. Analytics does not store tool arguments, responses, source text, debugger values, output panes, or messages. The "Keep history" value is optional: blank keeps analytics indefinitely, while a positive integer keeps that many daily buckets including today.
...
Usage analytics are stored separately from audit logs:

```text
%LOCALAPPDATA%\NetVsMcp\Analytics\analytics.db
```

Debug builds use a separate database beside the executable:

```text
<broker output folder>\Analytics\analytics.db
```

The broker stores daily aggregate buckets keyed by date, broker version, tool name, category, endpoint, and success/failure. Counters include calls, request/response character totals, estimated request/response token totals, total estimated tokens, total duration, and max duration. Weekly and monthly summaries are computed from daily rows when an agent calls `vs_get_usage_summary`.
